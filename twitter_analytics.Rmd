...
covid_all <- rbind.data.frame(covid, wuhanvirus, chinavirus, flattenthecurve, moderna,
                              pfizer, socialdistancing, stayathome)
                              #,coronavirus, covid19, vaccine
black <- rbind.data.frame(blm, breonnataylor, georgefloyd, racism, solidarity, sayhername, 
                          sayhisname, supportblackbusiness, blacklivesmatter)
asian <- rbind.data.frame(stopasianhate, chinavirus, aapivisibility, stopaapihate, 
...
covid_all$text <- clean_tweets(covid_all$text)
black$text <- clean_tweets(black$text)
asian$text <- clean_tweets(asian$text)
```
...
set.seed(1234)
wordcloud(asian_tweets$word, min.freq=10, max.words = 100, scale=c(2, .5), random.order=FALSE,
          rot.per=0.25, colors=brewer.pal(15, "Dark2"))

# Show the most frequent words associated with the Black experience 
set.seed(1234)
wordcloud(black_tweets$word, min.freq=5, max.words = 50, scale=c(2, .5), random.order=FALSE,
          rot.per=0.25, colors=brewer.pal(15, "Dark2"))
```
